...
async function analyzeProduct(product, expectations) {
    try {
...
async function quickSummary(product) {
    try {
...
async function compareProducts(product1, product2, priorities) {
    try {
    ...
    try {
        switch (name) {
            ...
            case "compare_products": {
                ...
                const result = await compareProducts(validated.product1, validated.product2, validated.priorities);
                return {
                    content: [{ type: "text", text: result }],
                };
            }
