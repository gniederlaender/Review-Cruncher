    ...
    const emailContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            ${searchResults && searchResults.length > 0 ? `
                <h3 style="color: #2563eb;">Top YouTube Review Videos</h3>
                <ul style="list-style: none; padding: 0;">
                    ${searchResults.map(result => `
                        <li style="margin-bottom: 20px; padding: 15px; background: #f1f5f9; border-radius: 8px;">
                            <a href="${result.link}" style="color: #2563eb; text-decoration: none; font-weight: bold;">
                                ${result.title}
                            </a>
                            <p style="margin: 10px 0; color: #475569;">${result.snippet}</p>
                        </li>
                    `).join('')}
                </ul>
            ` : ''}
...
app.post('/api/combined', async (req, res) => {
    try {
        ...
        const [recommendationResponse, searchResponse] = await Promise.all([
            ...
            axios.get(`https://www.googleapis.com/customsearch/v1?q=${encodeURIComponent(`${product} Youtube product review`)}&key=${GOOGLE_API_KEY}&cx=${GOOGLE_CSE_ID}`)
        ]);
